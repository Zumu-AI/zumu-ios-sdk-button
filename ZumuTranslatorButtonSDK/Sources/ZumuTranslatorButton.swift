...
public struct ZumuTranslatorButton: View {
    ...
    public init(config: ZumuTranslator.TranslationConfig, apiKey: String, shouldAutoStart: Bool = false) {
        self.config = config
        self.apiKey = apiKey
    ...
    public var body: some View {
        ...
        .onAppear {
            // Only auto-start once per button instance, and only if not already connected
            if shouldAutoStart && !hasAutoStarted && viewModel.state == .inactive && !viewModel.isConnected {
                print("🚀 Button: Auto-start triggered for trip: \(config.tripId)")
                hasAutoStarted = true
                Task {
                    try? await Task.sleep(nanoseconds: 500_000_000) // 0.5s delay for smooth UX
                    // Double-check state before connecting
                    guard viewModel.state == .inactive && !viewModel.isConnected else {
                        print("⚠️ Button: Auto-start aborted - state changed during delay")
                        return
                    }
    ...
    private var visualizationView: some View {
        ...
        case .inactive:
            // AI Translate icon - language/translation symbol
            LanguageIconView(size: 20, color: .white.opacity(0.9))
        ...
        case .listening:
            ...
            BarAudioVisualizer(
                ...
                barColor: .white, // White bars to match button text
                barCount: 5,
                barSpacingFactor: 0.08, // LiveKit SDK best practice value
                barMinOpacity: 1.0 // Solid bars (no grey effect)
            )
...
class ButtonViewModel: NSObject, ObservableObject, RoomDelegate {
    ...
    func connect() async {
        ...
        guard !isConnected && state == .inactive && !hasAttemptedConnection else {
            print("⚠️ Button: Connection blocked - isConnected: \(isConnected), state: \(state), hasAttempted: \(hasAttemptedConnection)")
            return
        }

        print("✅ Button: Starting connection - trip_id: \(config.tripId)")
        ...
        connectionTask = Task { @MainActor in
            ...
            do {
                ...
                // Log analytics fields (DEBUG: Show nil vs value)
                print("📞 External Driver ID: \(config.externalDriverId ?? "NIL - NOT SET")")
                print("📞 Member ID: \(config.memberId ?? "NIL - NOT SET")")
                print("📞 Pickup Location: \(config.pickupLocation ?? "NIL")")
                print("📞 Dropoff Location: \(config.dropoffLocation ?? "NIL")")
    ...
    func disconnect() async {
        ...
        hasAttemptedConnection = false  // Reset for next connection
